...
int x =0,y=0,click=0,vertices=5;
...
void compute(float u, int *xc,int *yc)
{
	...
	for(int k = 0 ; k <= vertices ; k++)
	{
		...
		blend= coeff[k] * (float)pow(u,k) * (float)pow(1-u,vertices-k);
...
int main(int argc, char **argv){
	...
	glutCreateWindow("Bezier Leaf"); //Name of the Window
	Init();
	//createMenu();
	glutMouseFunc(mouse);
